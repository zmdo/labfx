...
public class StaffGauge extends Pane{
	...
	@FXML private Canvas ruleCanvas;
	@FXML private Canvas markLayer;
	...
	private void init(
		...
		//初始化内部监听器
		posListeners = new HashSet<PositionChangeListener>();
		
		//绑定fxml
        try {
			LayoutBinder.bind(StaffGauge.class,"staff-gauge.fxml", this, this).load();
		} catch (IOException e) {
			e.printStackTrace();
		}
